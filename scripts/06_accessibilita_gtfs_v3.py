...
def hhmmss_to_seconds(value: object) -> float:
    """Converte un orario GTFS HH:MM:SS in secondi.

    Accetta anche ore oltre 24 (formato GTFS). Valori vuoti o malformati
    restituiscono NaN e vengono esclusi successivamente.
    """
...
def select_extraurban_routes_spatial(
    ...
    selection_area = boundary.geometry.union_all().buffer(buffer_m)
...
def load_feed(
    ...
    # In alcuni feed GTFS departure_time è vuoto per fermate non-timepoint.
    # Usiamo arrival_time come fallback e scartiamo solo gli orari ancora invalidi.
    dep_text = st.get("departure_time", pd.Series("", index=st.index)).astype(str).str.strip()
    arr_text = st.get("arrival_time", pd.Series("", index=st.index)).astype(str).str.strip()
    st["event_time"] = dep_text.where(dep_text.ne(""), arr_text)
    st["dep_s"] = st["event_time"].map(hhmmss_to_seconds)
    invalid_times = int(st["dep_s"].isna().sum())
    if invalid_times:
        print(
            f"Feed {spec.label}: escluse {invalid_times} righe di stop_times "
            "con orario vuoto o non valido"
        )
