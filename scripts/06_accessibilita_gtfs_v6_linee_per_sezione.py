...
def aggregate_lines_by_section(
    sections: gpd.GeoDataFrame,
    raw_stops: gpd.GeoDataFrame,
) -> pd.DataFrame:
    """
    Restituisce, per ogni sezione, i numeri delle linee urbane ed
    extraurbane che hanno almeno una fermata fisicamente interna.

    Usa le fermate originali dei feed, non i cluster aggregati, così ogni
    fermata conserva correttamente le linee che la servono.
    """
    section_index_name = "__section_index__"
    sections_tmp = sections[[sections.geometry.name]].copy()
    sections_tmp[section_index_name] = sections.index
    sections_tmp = sections_tmp.rename_geometry("section_geometry")

    stops_tmp = raw_stops[
        ["stop_id", "stop_name", "feed_source", "linee", "geometry"]
    ].copy()

    joined = gpd.sjoin(
        stops_tmp,
        sections_tmp.set_geometry("section_geometry"),
        how="inner",
        predicate="within",
    )

    def collect_lines(values: pd.Series) -> str:
        result = set()
    ...
    def collect_stop_names(values: pd.Series) -> str:
        result = sorted({v.strip() for v in values.fillna("").astype(str) if v.strip()})
        return " | ".join(result)

    rows = []
    for section_index, group in joined.groupby(section_index_name):
        urban = group[group["feed_source"] == "urbano"]
        extra = group[group["feed_source"] == "extraurbano"]

        rows.append(
            {
                "section_index": section_index,
                "linee_urbane_con_fermata_nella_sezione": collect_lines(
                    urban["linee"]
                ) if not urban.empty else "",
                "linee_extraurbane_con_fermata_nella_sezione": collect_lines(
                    extra["linee"]
                ) if not extra.empty else "",
                "fermate_urbane_nella_sezione": collect_stop_names(
                    urban["stop_name"]
                ) if not urban.empty else "",
                "fermate_extraurbane_nella_sezione": collect_stop_names(
                    extra["stop_name"]
                ) if not extra.empty else "",
                "n_fermate_urbane_nella_sezione": int(
                    urban["stop_id"].nunique()
                ) if not urban.empty else 0,
                "n_fermate_extraurbane_nella_sezione": int(
                    extra["stop_id"].nunique()
                ) if not extra.empty else 0,
            }
        )
    ...
    if not rows:
        return pd.DataFrame(
            ...
            columns=[
                "linee_urbane_con_fermata_nella_sezione",
                "linee_extraurbane_con_fermata_nella_sezione",
                "fermate_urbane_nella_sezione",
                "fermate_extraurbane_nella_sezione",
                "n_fermate_urbane_nella_sezione",
                "n_fermate_extraurbane_nella_sezione",
            ],
        )

    result = pd.DataFrame(rows).set_index("section_index")
    return result.reindex(sections.index)
...
def main() -> int:
    ...
    # Linee che hanno almeno una fermata realmente interna a ciascuna sezione.
    lines_by_section = aggregate_lines_by_section(sections, raw_stops)
    ...
    sections_out = sections_out.join(lines_by_section)
    representative_out = representative_out.join(lines_by_section)
    ...
    for column in [
        ...
        sections_out[column] = sections_out[column].fillna("")
        representative_out[column] = representative_out[column].fillna("")

    for column in [
        "n_fermate_urbane_nella_sezione",
        "n_fermate_extraurbane_nella_sezione",
    ]:
        sections_out[column] = sections_out[column].fillna(0).astype(int)
        representative_out[column] = representative_out[column].fillna(0).astype(int)
    ...
    # ------------------------------------------------------------------
    # Percorsi delle linee per la visualizzazione in uMap.
    # Mantiene soltanto le linee che attraversano il confine o hanno
    # almeno una fermata interna. Le geometrie delle singole corse vengono
    # aggregate per linea per evitare sovrapposizioni duplicate.
    # ------------------------------------------------------------------
    internal_route_ids = set(
        routes_summary.loc[
            routes_summary["interseca_confine"]
            | routes_summary["ha_fermata_interna"],
            "route_id",
        ].astype(str)
    )

    map_lines = route_lines[
        route_lines["route_id"].astype(str).isin(internal_route_ids)
    ].copy()

    if not map_lines.empty:
        map_lines = (
            map_lines[
                [
                    "feed_source",
        ...
        map_lines = map_lines.merge(
            route_info,
            on=["feed_source", "route_id", "route_short_name"],
            how="left",
        )
        map_lines = gpd.GeoDataFrame(
            map_lines,
            geometry="geometry",
            crs=route_lines.crs,
        )
        ...
        map_lines.to_file(
            gpkg,
            layer="linee_gtfs_nella_circoscrizione",
            driver="GPKG",
        )

        map_lines_wgs84 = map_lines.to_crs("EPSG:4326")
        map_lines_wgs84.to_file(
            out_dir / "linee_gtfs_nella_circoscrizione.geojson",
            driver="GeoJSON",
        )

        urban_map_lines = map_lines_wgs84[
            map_lines_wgs84["feed_source"] == "urbano"
        ].copy()
        extraurban_map_lines = map_lines_wgs84[
            map_lines_wgs84["feed_source"] == "extraurbano"
        ].copy()

        if not urban_map_lines.empty:
            urban_map_lines.to_file(
                out_dir / "linee_urbane_nella_circoscrizione.geojson",
                driver="GeoJSON",
            )

        if not extraurban_map_lines.empty:
            extraurban_map_lines.to_file(
                out_dir / "linee_extraurbane_nella_circoscrizione.geojson",
                driver="GeoJSON",
            )

    # GeoJSON destinato a uMap: sempre WGS84.
    if sections_out.crs is None:
        raise ValueError("Il layer delle sezioni non ha un CRS definito.")
    sections_out.to_crs("EPSG:4326").to_file(
