...
LAT_CANDIDATES = ("latitude", "latitudine", "lat", "y")
LON_CANDIDATES = ("longitude", "longitudine", "lon", "lng", "x")
GEOM_CANDIDATES = ("geometry", "geom", "wkt", "the_geom", "geo_shape", "geometria")
...
def read_poi(path: Path) -> pd.DataFrame:
    ...
    if suffix == ".csv":
        # sep=None usa il motore Python per rilevare automaticamente virgola,
        # punto e virgola o tabulazione.
        df = pd.read_csv(
        ...
        if lat_col and lon_col:
            ...
            # Alcuni CSV geografici salvano le coordinate in una colonna
            # geometry/WKT invece che in due colonne separate.
            geom_col = find_col(df.columns, GEOM_CANDIDATES)
            if not geom_col:
                raise ValueError(
                    f"{path}: coordinate non trovate. Colonne disponibili: "
                    f"{list(df.columns)}"
                )

            def parse_geometry(value: object):
                ...
                try:
                    ...
                    return shape(obj) if isinstance(obj, dict) else None
                except Exception:
                    return None

            geoms = out[geom_col].map(parse_geometry)
            pts = geoms.map(
            ...
            out["longitude"] = pts.map(lambda p: p.x if p is not None else None)
            out["latitude"] = pts.map(lambda p: p.y if p is not None else None)
    else:
